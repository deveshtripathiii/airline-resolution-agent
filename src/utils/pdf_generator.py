...
def generate_claim_slip_pdf(
    ...
    # 1. Header Banner
    elements.append(Paragraph("✈️ SKYWAY AIRLINES", title_style))
    elements.append(Paragraph("Official Disruption Resolution Certificate & Claim Receipt", subtitle_style))
    ...
    meta_data = [
        ...
        [
            Paragraph("<b>Passenger Name:</b>", body_style), Paragraph(customer_name, bold_body),
    ...
    flight_data = [
        ...
        [
            ...
            Paragraph(delay_info or "Operational Cancellation / Rescheduling", body_style),
        ],
    ]
    ...
    # 4. Authorized Entitlements & Actions
    elements.append(Paragraph("Authorized Entitlements & Disruption Settlements", section_heading))
    
    action_rows = [
        [Paragraph("<b>Authorized Item / Policy Benefit</b>", bold_body), Paragraph("<b>Status & Execution Details</b>", bold_body)]
    ]
    
    if not actions_taken:
        action_rows.append([
            Paragraph("Disruption Care Review", body_style),
        ...
        for act in set(actions_taken):
            ...
            if "refund" in act_lower:
                ...
                action_rows.append([
                    ...
                    Paragraph("Airport dining pass credited to boarding reference for departure terminal.", body_style)
                ])
            elif "lounge" in act_lower:
                action_rows.append([
                    Paragraph("<b>Executive Departure Lounge Pass</b>", body_style),
                    Paragraph("Complimentary lounge access authorized at departure terminal.", body_style)
                ])
            elif "hotel" in act_lower:
                action_rows.append([
                    Paragraph("<b>Transit Hotel Accommodation</b>", body_style),
                    Paragraph("Transit accommodation authorized covering the delayed-hours duration only.", body_style)
                ])
    ...
    # 5. Terms & Compliance Statement
    elements.append(Paragraph("Service Policy Terms & Compliance", section_heading))
    terms_text = (
        ...
        "are non-transferable and tied directly to the referenced PNR. For any supervisor follow-up, "
        "quote your Certificate Reference number at the airport customer care desk."
    )
    elements.append(Paragraph(terms_text, body_style))
    elements.append(Spacer(1, 12))

    # 6. Digital Stamp & Seal
    seal_data = [
        [
            Paragraph("<b>Issued By:</b><br/>SkyWay Autonomous Care Agent<br/>System Node: AI-AERO-RES-2026", body_style),
            Paragraph("<b>Duty Operations Seal:</b><br/>[VERIFIED & RECORDED]<br/>Audit Hash: OK-GND-23SEP", body_style),
        ]
    ]
